 ...
 * Converts Classic Nav Menus into Gutenberg FSE `wp_navigation` posts based on `inc/menus.json`.
...
$json_path = dirname(__DIR__) . '/inc/menus.json';
...
/**
 * Build block markup recursively from hierarchical menu items.
 */
function eka_build_nav_blocks_markup(array $items, int $parent_id = 0): string
{
    $markup = '';
...
foreach ($config['menu_groups'] as $group_key => $group_data) {
    ...
    foreach ($translations as $lang => $trans_info) {
        $classic_id      = $trans_info['classic_menu_id'] ?? 0;
        ...
        if ($classic_id > 0) {
            ...
            if ($items && !is_wp_error($items)) {
                $block_content = eka_build_nav_blocks_markup($items, 0);
            }
        }
