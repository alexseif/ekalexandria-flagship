...
- **Legacy Slider & Shortcode Mitigation:** 
  ...
  - **Static Revolution Sliders:** Utilize the mapping in `ai-work/scopings/legacy_data.md` alongside the `rev_sliders` array in `ai-work/scopings/legacy-ids.json`. The legacy inner pages (e.g., Cemeteries, Museums, Community Lounge) use Revolution Sliders for static image galleries. The migration script must programmatically inject native `core/gallery` blocks into these specific Page IDs using the exact pre-extracted Media IDs listed in `ai-work/scopings/legacy_data.md` (e.g., inserting Image IDs `7821, 7822, 7823` into the Music Museum pages). This bypasses the need to parse the Revolution Slider database entirely.
...
- **Execution Flow / Sequential Task Protocol (Script-Driven):** 
  1. **Environmental Reset (Pause Checkpoint):** Utilize `reset-env.sh` to synchronize files and DB dump from local production copy. Remove new files from previous failed solution attempts while **strictly preserving** `ai-work/` (containing specs, scopings, baselines, and `legacy_data.md`) and `Master Project Roadmap...`. To prevent the "Mailchimp composer cache error" logged in `debug.log`, clear/deactivate the mailchimp plugin vendor autoload or exclude it during sync. **Critical Pause Checkpoint:** After reset-env completes, HALT execution and wait for explicit manual user approval before moving forward.
  2. **Orchestration Layer Setup:** Verify or initialize `@wordpress/scripts`, Playwright, and AST parser (reusing existing setup if present).
  3. **Scoping & Baseline Extraction (Pause Checkpoint):** Run Playwright script (`bin/scrape-baselines.js`) with `ignoreHTTPSErrors: true` and proper linguistic URLs (`/el/`, `/en/`, `/ar/`) to ensure no 404s. **Critical Pause Checkpoint:** After screenshots are taken, HALT execution and present status to the user with options to either **Proceed** or **Rerun Screenshot Scrape**.
