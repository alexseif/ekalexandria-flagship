...
- [x] **Task 1: Refactor `step_4c_transform_vc_posts_grid()` in `bin/04-shortcode-migrations.php`**
  - [x] Update function signature to `step_4c_transform_vc_posts_grid($content, $post_id = 0)`.
  - [x] Extract `by_id:ID1,ID2...` from `loop="..."` shortcode attribute into `"include": [ID1, ID2, ...]`.
  - [x] Inject `"parents": [$post_id]` if `$post_id > 0`.
  - [x] Build AST-compliant 2-column `wp:query` block wrapped in `wp:group` with 4:3 aspect ratio, 12px rounded corner featured images, and level 3 linked titles.
  - [x] Handle wrapping `[vc_row]` / `[vc_column]` replacement around `[vc_posts_grid]`.
  - Verification: Test block conversion output against AST validator (`eka_validate_blocks_ast`).

- [x] **Task 2: Update Main Loop Execution Call Site**
  - [x] Pass current post `$id` into `step_4c_transform_vc_posts_grid($content, $id)` in `bin/04-shortcode-migrations.php`.
